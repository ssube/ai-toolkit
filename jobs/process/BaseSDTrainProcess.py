...
class BaseSDTrainProcess(BaseTrainProcess):
    ...
    def prepare_accelerator(self):
        ...
        if self.train_config.use_clauto_opt:
            ...
            clauto_kwargs = {k: v for k, v in self.train_config.clauto_opt_config.items() if k not in ('auto_stop', 'backend')}
            clauto_backend = self.train_config.clauto_opt_config.get('backend', 'api')
            clauto_config = ClaudeOptimizerConfig(
                total_steps=self.train_config.steps,
                **clauto_kwargs
            )
            self.optimizer = ClaudeOptimizer(self.optimizer, backend=clauto_backend, config=clauto_config)
